# Based on https://github.com/anki/cozmo-python-sdk/blob/master/examples/apps/desk_security_guard.py

'''Cozmo the Email Security Guard.
...
class EmailSecurityGuard:
    ...
    def has_confirmed_intruder(self):
        ...
        if self.time_first_observed_intruder:
            ...
            return elapsed_time > 5.0
        return False

def send_email(raw_image = None):
    global GMAIL_USER
    global APP_PASSWORD
    ...
    text = MIMEText("Intruder detected at %s" % datetime.datetime.now().strftime("%B %d, %Y %I:%M%p"))
    msg.attach(text)
    
    if raw_image != None:
        memf = io.BytesIO()
        raw_image.save(memf, "JPEG")
        image = MIMEImage(memf.getvalue(), name='intruder.jpg')
...
async def check_for_intruder(robot, dsg:EmailSecurityGuard):
    ''''''
    ...
    can_see_owner = did_occur_recently(dsg.time_last_observed_owner, 30.0)
    ...
    if can_see_owner:
        ...
        if latest_photo is None:
            latest_photo = robot.world.latest_image
        ...
        if is_confirmed_intruder:
            ...
            # Send email
            if not did_occur_recently(dsg.time_last_uploaded_photo, 60.0):
                if latest_photo is not None:
                    email_sent = send_email(latest_photo.raw_image)
                    latest_photo = None
                    print("Email sent with camera image.")
                else:
                    email_sent = send_email()
                    print("Email sent without camera image.")
                    
                if email_sent:
                    dsg.time_last_uploaded_photo = time.time()
                else:
                    print("Failed to email photo of intruder!")

            # Sound an alarm (every X seconds)
            if not did_occur_recently(dsg.time_last_announced_intruder, 10):
                await robot.say_text("Intruder Alert", use_cozmo_voice=False).wait_for_completed()
            ...
            # Possibly an intruder - turn backpack blue to indicate
...
async def run(sdk_conn):
    ...
    try:
        await email_security_guard(robot)
